...
def found(peripheral):
    connectable_str = (
        "Connectable" if peripheral.is_connectable() else "Non-Connectable"
    )
    print(
        f"Found {peripheral.identifier()} [{peripheral.address()}] ({connectable_str})"
    )
...
if __name__ == "__main__":
    ...
    for i, (service_uuid, characteristic) in enumerate(service_characteristic_pair):
        ...
        contents = peripheral.read(service_uuid, characteristic)
