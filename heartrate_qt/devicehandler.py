...
class DeviceHandler(BluetoothBaseClass):
    ...
    def __init__(self, parent=None):
        ...
        address = QBluetoothAddress("E8:BC:51:D0:95:6E")
        channel = 1
        print(f"Trying to connect to channel {channel}")
        self.socket.connectToService(address, channel)
        # for channel in range(1, 101):
        #     try:
        #         print(f"Trying to connect to channel {channel}")
        #         socket.connectToService(address, channel)
        #         time.sleep(5)
        #         if socket.state() == QBluetoothSocket.SocketState.ConnectedState:
        #             print(f"Connected to channel {channel}")
        #             break
        #         print(socket.errorString(), socket.state())
    ...
    def socketConnected(self):
        self.info = "SOCKET connected. Search services..."
        # self.m_control.discoverServices()
    ...
    def setDevice(self, device):
        ...
        if self.m_currentDevice:
            # Make connections
            #! [Connect-Signals-1]
            self.m_control = QLowEnergyController.createCentral(
                self.m_currentDevice.device(), self
            )
            #! [Connect-Signals-1]
    ...
    def serviceScanDone(self):
        ...
        if self.m_foundHeartRateService:
            self.m_service = self.m_control.createServiceObject(
                QBluetoothUuid(QBluetoothUuid.ServiceClassUuid.BatteryService), self
            )
        ...
        if switch == QLowEnergyService.RemoteServiceDiscovering:
            ...
            if hrChar.isValid():
                ...
                if self.m_notificationDesc.isValid():
                    self.m_service.writeDescriptor(
                        self.m_notificationDesc, QByteArray.fromHex(b"0100")
                    )
            else:
    ...
    def disconnectService(self):
        ...
        if (
            self.m_notificationDesc.isValid()
            and self.m_service
            and self.m_notificationDesc.value() == QByteArray.fromHex(b"0100")
        ):
            self.m_service.writeDescriptor(
                self.m_notificationDesc, QByteArray.fromHex(b"0000")
            )
        else:
            if self.m_control:
                self.m_control.disconnectFromDevice()
    ...
    def addMeasurement(self, value):
        ...
        if self.m_measuring and value > 30 and value < 250:
            ...
            self.m_calories = (
                (
                    (-55.0969 + (0.6309 * self.m_avg) + (0.1988 * 94) + (0.2017 * 24))
                    / 4.184
                )
                * 60
                * self.time
